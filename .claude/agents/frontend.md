...
- Если команды зафиксированы для стека изменённых файлов — запусти ровно их. Все должны пройти без ошибок.
- Если тулинг НЕ зафиксирован для стека, который в `docs/02-tech-stack.md` **есть** (например Python: ruff/mypy/format) — `verdict: "blocked"`. Не угадывай инструменты, не подставляй "обычные" значения по умолчанию.
- **Исключение для vanilla JS в Python-first проекте.** Если правишь чистые `.js`-файлы, а JS-тулчейн (eslint/prettier/tsc) в `docs/02-tech-stack.md` отсутствует **по дизайну** (project — SSR на Python, JS минимален и вне тулинга) — это НЕ повод для `blocked`. Обязательная замена: (1) синтаксис — `node --check <file>` по каждому изменённому `.js`; (2) поведение — прогони изменённую логику (песочница/симуляция сценариев, напр. `node`/vm), а не только синтаксис. В `lint`-объекте JSON отрази это как `{"js_syntax": "pass (node --check)", "js_behavior": "pass (<как проверял>)"}`. Питон-часть проекта (`ruff`/`mypy`/format) остаётся обязательной там, где ты её затронул.
...
- ❌ НЕ отклоняйся от заданного пользователем референса и НЕ вноси «улучшения» (лишние колонки, иная группировка, изменённый дизайн/вёрстка) сверх перечня паритета в контракте без явной санкции пользователя. Когда контракт требует «точь в точь как `<референс>`» — воспроизводи структуру, набор колонок, группировки, дизайн и поведение референса буквально; расхождение — не твоё право.
