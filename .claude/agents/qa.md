...
- **Form/filter submit follow-through (ОБЯЗАТЕЛЬНО):** для любой формы/фильтра (в т.ч. no-JS GET-формы) тест ОБЯЗАН отправить ФАКТИЧЕСКИЙ запрос, который форма генерирует, включая состояние по умолчанию с пустыми/дефолтными селектами («Все команды»/«Все номера»), которые сериализуются в пустые query-параметры (например `?to_number=&team_id=&limit=50`). Утверждай терминальный 200 И реальную фильтрацию результата под каждой релевантной ролью. Проверка SSR-контекста/рендера страницы без сабмита сгенерированного формой URL НЕДОСТАТОЧНА — пустой submit по дефолту обязан вернуть 200 (не 422/validation_error).
...
- **Logout stickiness (ОБЯЗАТЕЛЬНО):** тест logout доводится до конечного состояния — после выхода пользователь ОСТАЁТСЯ разлогинен. Если в проекте есть авто-SSO-glue (например `tg.js`, шлющий initData при каждой загрузке), тест обязан воспроизвести следующую загрузку страницы после logout и утвердить, что авто-логин НЕ восстанавливает сессию мгновенно. Проверка «logout вернул 302→/login» НЕДОСТАТОЧНА.
...
- [ ] Формы/фильтры протестированы фактическим сабмитом сгенерированного URL, включая дефолтный пустой submit («Все …» → `?param=&…`): терминальный 200 + реальная фильтрация, а не только рендер
- [ ] Пост-логин landing достижима под каждой ролью (super_admin / group_leader / group_member)
- [ ] Logout доведён до конечного состояния: после выхода следующая загрузка НЕ авто-логинит обратно (при наличии авто-SSO-glue)
