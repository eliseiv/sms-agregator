...
### `GET /admin`
- **Guard:** `require_admin`. Jinja2-страница: список пользователей (**единая таблица** с `<tbody>`-banding по командам — паритет с mail-agregator, [ADR-0015](./adr/ADR-0015-admin-users-visual-parity-with-mail-agregator.md)), поиск, пагинация, диалоги создания/перевода/добавления/удаления, назначение команды номерам, **секция распределения unassigned-номеров**. Использует `admin_users.js` + `csrf.js` (+ `admin_numbers.js`).
- **Query:** `q?: str` (поиск по логину, `max_length=64`), `page?: int` (`ge=1`, дефолт `1`), `limit?: int` (`ge=1, le=200`, дефолт `50`). Совпадают с референсом (`mail-agregator/backend/app/admin/router.py::admin_users_page`).

#### Паритет `/admin` (НОРМАТИВНО — приёмочное условие, [ADR-0015](./adr/ADR-0015-admin-users-visual-parity-with-mail-agregator.md))

Целевая визуально-структурная модель списка пользователей — **порт `mail-agregator/backend/app/templates/admin/users.html` + соответствующих блоков `static/css/main.css`**. Обязательно к соблюдению frontend; проверяется reviewer/QA.

**Колонки (единый `<thead>`, строго в порядке):** `Имя · Роль · Команда · Создан · Последний вход · Действия` (6). Колонки «Telegram» **нет**.

**Структура таблицы:**
- Одна `<table class="admin-users-table">` в обёртке `<div class="user-groups-list">` с единым `<thead>`.
- Команды — **`<tbody>`-бакеты** `user-group user-group--band-a` / `--band-b` (`data-group-id`), между бакетами — спейсер `<tbody class="user-group__spacer">`. Banding чередуется **только по командным бакетам** (namespace-счётчик); бакет «без команды» (super_admin) — нейтральный `user-group--no-group`, рендерится **первым**. **Per-group заголовков/счётчиков нет** (никаких `admin__group-head`/`-title`/`-count`, никакой отдельной секции «Администраторы»).
- **Один пользователь = одна строка** (`admin-users-table__row`; для super_admin — `... --system`) в бакете его **домашней** команды; super_admin — в бакете «без команды».

**Ячейки строки:**
- **Имя** (`admin-users-table__name`): `admin-users-table__name-row` → `admin-users-table__name-text` (эфф. имя: `display_name` или `username`) + для НЕ-super_admin кнопка «+» (`admin-users-table__add-group`, `data-user-id`/`data-username`/`data-current-gid`/`data-is-leader`/`data-member-gids`) — открывает меню-диалог управления командами.
- **Роль** (`admin-users-table__role`): бейдж `admin-user__badge admin-user__badge--{role}` с `role_label`.
- **Команда** (`admin-users-table__group`): **чипы всех команд** пользователя (`team-chips` → `team-chip`, домашняя — `team-chip--home`). Домашний чип — **без** контрола удаления; каждый доп.-чип несёт «×» (форма удаления, no-JS fallback: `POST /api/admin/users/{id}/teams/{team_id}` + `_method=DELETE` + csrf, JS перехватывает — §4). super_admin/без команды → `admin-users-table__muted` «без команды».
- **Создан / Последний вход** (`admin-users-table__date`): `YYYY-MM-DD HH:MM` (`time_tag_short`); нет входа → `admin-users-table__muted` «никогда».
- **Действия** (`admin-users-table__actions`): для НЕ-super_admin — **Сброс** (`POST /api/admin/users/{id}/reset`) + **Удалить** (диалог подтверждения вводом логина → `DELETE /api/admin/users/{id}`); для super_admin — `admin-users-table__muted` «системный». Перемещение/добавление в команду — **не** здесь, а через «+»-меню в ячейке имени.

**Тулбар** (`admin__toolbar`): заголовок «Пользователи» + `Команды →` (link `/admin/teams`), `+ Создать пользователя`, `+ Создать команду`. Ссылка `Журнал действий →` — **только если** в sms существует страница аудита (сейчас нет — вне scope этой итерации; добавляется вместе с /admin/audit при появлении).

**Поиск:** `<form method="GET" action="/admin" role="search">` с `<input name="q">` (placeholder «Поиск по логину», значение = текущий `q`) и кнопкой «Найти».

**Пагинация:** внизу — `page_pagination(page, total, limit, base_url)` (стрелки «Назад»/«Далее», «Стр. N из M»); `base_url` сохраняет `q`.
...
- **SSR-контекст (НОРМАТИВНО; split-ownership: шаблон `app/api/templates/admin/users.html` авторит frontend, контекст рендера передаёт backend).** Backend **ОБЯЗАН** инжектировать переменные под этими **ЛИТЕРАЛЬНЫМИ** именами (порт имён референса, с sms-терминологией `team`):
  - `user_groups: list[dict]` — бакеты в порядке сортировки. Каждый: `{team_id: int|None, team_name: str|None, users: list[<user>]}`. Строится **линейным проходом** по предсортированному списку пользователей (сортировка §4: super_admin/без-команды первыми → по домашней команде `team_name ASC` (стабильно `team_id`) → лидер первым (`is_leader DESC`) → `username ASC`); бакет ключуется по **домашней** команде (`users.team_id`; `None` для super_admin). Каждый `<user>`: `{id, username, display_name: str|None, role, home_team: {id, name}|None, memberships: list[{id, name}], created_at, last_login_at, password_reset_required: bool}`.
    - `home_team` — бриф домашней команды (`users.team_id`), `None` для super_admin. Домашний чип в шаблоне = `mem.id == home_team.id`.
    - `memberships` — **все** команды пользователя (home ∪ доп., из `user_teams`; bulk `UserTeamRepository.list_team_ids_for_users`), для рендера чипов. Пользователь показывается **одной строкой** в home-бакете; доп. членства — чипами, **НЕ** отдельными строками/бакетами.
  - `teams: list[{id, name, leader_user_id}]` — все команды (для `<select>` создания/перевода/добавления членства/назначения номерам и для вычисления команд-без-лидера в диалоге создания). Может быть пустым.
  - `q: str` — текущий поисковый запрос (значение поля поиска), дефолт `""`.
  - `total: int`, `page: int`, `limit: int` — для пагинации (`total` — общее число совпавших пользователей).
  - `csrf_token: str`, `is_super_admin: true`.
  - **Telegram-привязка (`has_telegram_link`) — НЕ отображается на `/admin` вовсе** (финальное решение заказчика по Q-ADMIN-1, вариант «a» — чистый паритет с mail-agregator): ни колонки, ни бейджа, ни иконки. Поле `has_telegram_link` **не входит** в `<user>` SSR-контекста `/admin` и backend его сюда **не** инжектирует (это не заглушка, а окончательный контракт). Данные привязки остаются в БД/других endpoint'ах — но на этой странице не показываются.
  - **Banding НЕ входит в контекст** — класс банда (`user-group--band-a/-b`, `--no-group`) вычисляется **в шаблоне** чередованием по `user_groups` (namespace-счётчик; см. [03-architecture.md](./03-architecture.md) §«Подсветка команд»). CSP-safe (только CSS-классы), схема БД не меняется.
  - Данные согласованы с `GET /api/admin/users` (§4, тот же набор + `team_ids`/`is_leader` + порядок сортировки); `user_groups` — представление того же набора. Отдельного grouping-API нет — SSR-инжект.
