...
| `Q-ADMIN-1` | При удалении колонки «Telegram» на `/admin` ради паритета с mail-agregator ([ADR-0015](./adr/ADR-0015-admin-users-visual-parity-with-mail-agregator.md)) — куда деть индикатор Telegram-привязки (`has_telegram_link`), которого нет в референсе? | **resolved** (2026-07-04) | Среднее | **Решение заказчика: вариант (a) — убрать индикатор полностью.** На `/admin` индикатор Telegram-привязки не отображается **вовсе**: ни колонки, ни бейджа, ни иконки. `has_telegram_link` не входит в строку пользователя SSR-контекста `/admin` (§7). Финально (не заглушка). |
