...
- Logout: `DEL session:{token}` + clear cookies (`sms_session`/`sms_csrf`); `telegram_links` **не трогаются** ([ADR-0004](./adr/ADR-0004-telegram-mini-app-sso.md)). Дополнительно ставится маркер `sms_logged_out` (не HttpOnly, `Secure`/`SameSite=Lax`, TTL `LOGOUT_STICKY_TTL_SECONDS`) — подавляет авто-SSO Mini App до явного входа; не секрет и не влияет на push ([ADR-0011](./adr/ADR-0011-sticky-logout-vs-miniapp-sso.md)). Маркер сбрасывается при явном входе (кнопка «Войти» на `/login`) или установлении новой сессии; TTL — safety-net.
...
| `POST /api/telegram/webhook` | напр. 60/min per IP (до проверки секрет-токена) | IP |
...
- **Новый бот ([ADR-0010](./adr/ADR-0010-telegram-webhook-and-new-bot.md)):** `TELEGRAM_BOT_TOKEN` — токен нового бота, используется как ключ HMAC (шаг 3 выше) и для `sendMessage`. При смене токена ранее выданные initData перестают валидироваться (ожидаемо).

## 6. Анти-энумерация логинов (мягкая — амендмент ADR-0002)

> **Изменение (2026-07-02, амендмент [ADR-0002](./adr/ADR-0002-two-step-login.md)).** Прежняя «жёсткая» анти-энумерация делала шаг-1 безусловным `303 → /login/password`, из-за чего новый пользователь (без пароля / `password_reset_required`) не мог пройти ТЗ-флоу «придумай пароль». По решению владельца приоритет отдан ТЗ-флоу первого входа; анти-энумерация шага-1 ослаблена до **мягкой**. Согласуется с принятым [TD-010](./100-known-tech-debt.md).

- `POST /login` (шаг-1) ветвится по состоянию аккаунта: `password_hash IS NULL`/`password_reset_required=true` → `/set-password` (+`sms_setup`); иначе → `/login/password` (+`sms_login`).
- **Что сохраняется:** несуществующий логин отвечает **идентично активному** аккаунту с паролем (`303 → /login/password`); существование активного аккаунта **не раскрывается**. Шаг-2 для несуществующего логина даёт общий `invalid_credentials` (anti-timing dummy-hash).
- **Что раскрывается (принятый риск):** различимо только состояние первичной активации (`password_reset_required` → `/set-password`). Это ровно риск TD-010 (§6a) — актор, знающий логин в состоянии активации, и раньше мог занять аккаунт; правка укорачивает путь на шаг, не добавляя нового класса угрозы. Компенсация — rate-limit `/login`+`/set-password` (§4), короткое окно активации, секретность логина.
...
**Остаточный риск (принят владельцем, [100-known-tech-debt.md](./100-known-tech-debt.md) TD-010):** актор, знающий существующий логин в состоянии `password_reset_required=true`, может занять неактивированный аккаунт в окне между созданием (или admin reset) и первым легитимным входом. После амендмента [ADR-0002](./adr/ADR-0002-two-step-login.md) (2026-07-02) шаг-1 `POST /login` направляет такой аккаунт сразу на `/set-password` — путь короче на один шаг, но класс риска тот же (claim неактивированного аккаунта уже был возможен через шаг-2). Владелец подтвердил приоритет ТЗ-флоу первого входа над жёсткой анти-энумерацией шага-1.
...
- Все секреты (`ADMIN_PASSWORD`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, `TWILIO_AUTH_TOKEN`, `DATABASE_URL`, `REDIS_URL`) — из env, не в коде/репозитории.
...
- **Webhook (`/api/telegram/webhook`):** тело апдейта, `X-Telegram-Bot-Api-Secret-Token` и бот-токен **не** логируются (§11).
...
| Подмена Telegram-webhook | Секрет-токен `X-Telegram-Bot-Api-Secret-Token` (constant-time, §11) + rate-limit по IP. |
| Подмена/replay initData | HMAC bot-token + TTL 5 мин + constant-time compare. |
| Brute-force пароля | Rate-limit + lockout + argon2id. |
| Энумерация логинов | Мягкая анти-энумерация: несуществующий логин неотличим от активного (`→ /login/password`), anti-timing dummy-hash на шаге-2. Различимо только состояние активации (`→ /set-password`) — принятый риск TD-010 (§6, §6a). |
| Мгновенный авто-relogin после logout в Mini App | Маркер `sms_logged_out`: `tg.js` не делает авто-SSO, `/api/telegram/auth` не создаёт сессию при маркере; сброс — явным входом ([ADR-0011](./adr/ADR-0011-sticky-logout-vs-miniapp-sso.md)). |
...
## 11. Telegram webhook — секрет-токен и отсутствие polling

Источник — [ADR-0010](./adr/ADR-0010-telegram-webhook-and-new-bot.md).

- **Аутентификация вебхука** — заголовок `X-Telegram-Bot-Api-Secret-Token`, задаётся при `setWebhook(secret_token=TELEGRAM_WEBHOOK_SECRET)`. Обработчик `POST /api/telegram/webhook` сверяет его с `TELEGRAM_WEBHOOK_SECRET` (constant-time compare) **до** разбора тела; несовпадение/отсутствие → `403 invalid_webhook_secret`. `TELEGRAM_WEBHOOK_SECRET` — случайная строка ≥32 символов, только из env.
- **CSRF-exempt** — сессии/`sms_csrf` на вебхуке нет; защита — секрет-токен + rate-limit по IP (§4).
- **Нет long polling** — апдейты приходят только по HTTPS-вебхуку за edge-nginx ([ADR-0005](./adr/ADR-0005-sms-addressing-via-team.md), [ADR-0007](./adr/ADR-0007-deploy-behind-shared-edge-nginx.md)); фоновый polling-loop не запускается.
- **Минимум логики / логов** — бот отвечает только на `/start` (кнопка `web_app`); прочие апдейты — no-op `200`. Тело апдейта и токены не логируются (§9).
